...
# covariateData = [fVec2, eVec2, cVec2]
# covariates = "F"
...
def optimizeSolution(fd, B):
    ...
    # sol = scipy.optimize.root(fd, x0=B, method='lm')
...
def experiment(iterations, dataset_string, model_string, cov_string):
    ...
    for loop in range(iterations):
        ...
        # solve for beta
        beta0, infodict, beta_conv, mesg = scipy.optimize.fsolve(initial_beta_function, x0=betaEstimate, full_output=True)
        # beta0 = scipy.optimize.brentq(initial_beta_function, a=0.0, b=1.0)
        # initial = scipy.optimize.fsolve(objective_function, x0=initial)

        # if beta_conv == 1:
        #     print("beta estimate converged")
        # else:
        #     print("beta estimate DID NOT converge")
        ...
        sol, convergence = optimizeSolution(f, [0.01, 0.01])
        ...
        # rows[loop] = [dataset_string, model_string, cov_string, b, betas, np.mean(np.array(elapsed_time)), llfVal, converged]

    row = [dataset_string, model_string, cov_string, b, betas, np.mean(np.array(elapsed_time)), llfVal, converged]
    print("calculated LLF =", llfVal)
    print("converged:", converged)
    print(np.mean(np.array(elapsed_time)))
