...
for loop in range(iterations):
    ...
    b = symengine.diff(f, x[0])
    lambda_b = symengine.lambdify(x, [b], backend='lambda')
    ...
    # beta0 = scipy.optimize.brentq(initial_beta_function, a=-0.5, b=0.5)
    ...
    # x = np.linspace(-1.0, 1.0, num=250)

    # fig, ax = plt.subplots()
    # ax.plot(x, [initial_beta_function(x[i]) for i in range(250)])
